...
public class FSHFenceListener extends FenceBroadcastReceiver implements FenceBroadcastReceiver.FenceListener {

    private static final String FENCE_NAME = "LOCATION_FENCE";
    public static final double FSH_LATITUDE = 47.653437;
    public static final double FSH_LONGITUDE = -122.316338;
    public static final double FSH_RADIUS = 100;
    public static final long DWELL_TIME_MILLIS = 1;

    private final PrependableTextView mUpdate;
    private final ImageView imageView;

    public FSHFenceListener(AwarenessFence duringFence, AwarenessFence startFence,
    ...
    public void duringFence(FenceState state) {
        switch (state.getCurrentState()) {
            case FenceState.TRUE:
                mUpdate.prependText(R.string.text_at_FSH);
                imageView.setImageResource(R.mipmap.fsh);
                break;
            case FenceState.FALSE:
                mUpdate.prependText(R.string.text_not_at_FSH);
    ...
    public void startingFence(FenceState state) {
        switch (state.getCurrentState()) {
            case FenceState.TRUE:
                mUpdate.prependText(R.string.text_entering_FSH);
                imageView.setImageResource(R.mipmap.fsh);
                break;
    ...
    public void endingFence(FenceState state) {
        switch (state.getCurrentState()) {
            case FenceState.TRUE:
                mUpdate.prependText(R.string.text_exiting_FSH);
                imageView.setImageDrawable(null);
                break;
            case FenceState.UNKNOWN:
                Log.i(TAG, "Unknown fence state");
